...
export const Carousel = React.forwardRef<HTMLDivElement, CarouselProps>(
  ({ slides, className = '', slideClassName = '', onSlideChange }, ref) => {
    ...
    // Update data attributes for the current slide's yScroll element
    useEffect(() => {
      if (!carouselRef.current) return;

      // Since we only render the current slide, find its yScroll element
      const currentSlideElement = carouselRef.current.querySelector('[data-slide-index]') as HTMLElement;
      if (currentSlideElement) {
        const yScrollElement = currentSlideElement.querySelector('.yScroll') as HTMLElement;

        if (yScrollElement) {
          yScrollElement.setAttribute('data-visible', 'true');
          yScrollElement.setAttribute('data-activate-at', String(Date.now() + 250));
        }
      }
    }, [currentIndex]);
      ...
      >
        ...
        <div ref={carouselRef} className={styles.carouselContent}>
          {slides[currentIndex] && (
            ...
            >
              <div className={styles.slideContent}>{slides[currentIndex].content}</div>
            </div>
          )}
        </div>
